...
def main() -> None:
    ...
    assert isotope_evidence_result["record_count"] == 79, isotope_evidence_result
    ...
    assert unresolved_isotope_evidence_result["record_count"] == 88, (
        unresolved_isotope_evidence_result
    )
    ...
    assert atom_behavior_result["profile_count"] == 79, atom_behavior_result
    assert atom_behavior_result["neutral_profile_count"] == 79, atom_behavior_result
    assert atom_behavior_result["stable_isotope_profile_count"] == 74, atom_behavior_result
    ...
    assert atom_behavior_gap_result["receipt_count"] == 88, atom_behavior_gap_result
    assert atom_behavior_gap_result["isotope_only_gap_count"] == 24, atom_behavior_gap_result
    ...
    assert atom_behavior_gap_work_result["work_item_count"] == 88, (
        atom_behavior_gap_work_result
    )
    assert atom_behavior_gap_work_result["isotope_evidence_required_count"] == 24, (
        atom_behavior_gap_work_result
    )
    assert atom_behavior_gap_work_result["seed_and_matter_profile_required_count"] == 64, (
        atom_behavior_gap_work_result
    )
    gallium_atom_gap = get_atom_behavior_gap_receipt("Ga")
    radon_atom_work = get_atom_behavior_gap_work_item("Rn")
    assert gallium_atom_gap.profile_blockers == ("isotope_evidence",), gallium_atom_gap
    ...
    assert element_readiness_result["ready_count"] == 30, element_readiness_result
    assert element_readiness_result["blocked_by_isotope_evidence_count"] == 24, (
        element_readiness_result
    )
    assert element_readiness_result["blocked_by_seed_and_matter_count"] == 64, (
        element_readiness_result
    )
    assert element_readiness_result["high_priority_gap_count"] == 24, (
        element_readiness_result
    )
    ...
    gallium_readiness = get_element_readiness_score("Ga")
    ...
    assert gallium_readiness.readiness_status == (
        "atom_behavior_blocked_by_isotope_evidence"
    ), gallium_readiness
    assert gallium_readiness.gap_priority_score == 1.0, gallium_readiness
    ...
    assert isotope_source_policy_result["policy_count"] == 24, (
        isotope_source_policy_result
    )
    ...
    gallium_isotope_policy = get_isotope_source_policy("Ga")
    assert gallium_isotope_policy.target_atom_behavior_gap_receipt_id == (
        "MSPEE-ATOM-BEHAVIOR-GAP-Z031-Ga"
    ), gallium_isotope_policy
    assert gallium_isotope_policy.atom_behavior_generation_allowed is False, (
        gallium_isotope_policy
    )
    ...
    assert isotope_source_search_result["search_receipt_count"] == 24, (
        isotope_source_search_result
    )
    assert isotope_source_search_result["open_search_count"] == 24, (
        isotope_source_search_result
    )
    ...
    gallium_isotope_search = get_isotope_source_search_receipt("Ga")
    assert gallium_isotope_search.policy_id == gallium_isotope_policy.policy_id, (
        gallium_isotope_search
    )
    assert gallium_isotope_search.search_status == "isotope_source_search_open", (
        gallium_isotope_search
    )
    assert gallium_isotope_search.candidate_receipt_id is None, gallium_isotope_search
    ...
    assert isotope_candidate_admission_result["receipt_count"] == 1, (
        isotope_candidate_admission_result
    )
    assert isotope_candidate_admission_result["admitted_to_canonical_count"] == 1, (
        isotope_candidate_admission_result
    )
    assert isotope_candidate_admission_result["admitted_isotope_count"] == 3, (
        isotope_candidate_admission_result
    )
    ...
    gallium_candidate_template = build_isotope_candidate_evidence_template("Ga")
    assert gallium_candidate_template["symbol"] == "Ga", gallium_candidate_template
    assert gallium_candidate_template["seed_mutation_allowed"] is False, (
        gallium_candidate_template
    )
    ...
    gallium_unresolved_isotope = find_unresolved_isotope_evidence_record("Ga")
    ...
    assert gallium_unresolved_isotope.evidence_domain == "isotope_evidence", (
        gallium_unresolved_isotope
    )
    ...
    api_gallium_unresolved_isotope = handle_api_request(
        "GET",
        "/evidence/isotopes/unresolved/Ga",
    )
    ...
    api_gallium_isotope_source_policy = handle_api_request(
        "GET",
        "/atom/behavior/isotope-source-policy/Ga",
    )
    api_gallium_isotope_source_search = handle_api_request(
        "GET",
        "/atom/behavior/isotope-source-search/Ga",
    )
    ...
    api_gallium_isotope_candidate_template = handle_api_request(
        "GET",
    ...
    assert api_health.payload["isotope_candidate_admission_receipt_count"] == 1, (
        api_health.payload
    )
    ...
    assert api_gallium_unresolved_isotope.status_code == 200, (
        api_gallium_unresolved_isotope
    )
    assert (
        api_gallium_unresolved_isotope.payload["record"]["evidence_domain"]
        == "isotope_evidence"
    )
    ...
    assert api_gallium_isotope_source_policy.status_code == 200, (
        api_gallium_isotope_source_policy
    )
    assert api_gallium_isotope_source_policy.payload["policy"]["symbol"] == "Ga", (
        api_gallium_isotope_source_policy.payload
    )
    assert api_gallium_isotope_source_policy.payload["policy"][
        "atom_behavior_generation_allowed"
    ] is False, api_gallium_isotope_source_policy.payload
    assert api_gallium_isotope_source_search.status_code == 200, (
        api_gallium_isotope_source_search
    )
    assert api_gallium_isotope_source_search.payload["receipt"]["symbol"] == "Ga", (
        api_gallium_isotope_source_search.payload
    )
    assert (
        api_gallium_isotope_source_search.payload["receipt"]["search_status"]
    ...
    assert api_gallium_isotope_candidate_template.status_code == 200, (
        api_gallium_isotope_candidate_template
    )
    assert api_gallium_isotope_candidate_template.payload["template"]["symbol"] == "Ga", (
        api_gallium_isotope_candidate_template.payload
    )
