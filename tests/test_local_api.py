...
def test_local_api_health_and_index_routes_are_read_only_contracts():
    ...
    assert "GET /atom/behavior/gaps/Rb" in index["routes"]
    assert "GET /atom/behavior/workplan/Rb" in index["routes"]
    assert "GET /atom/behavior/isotope-source-policy/Rb" in index["routes"]
    assert "GET /atom/behavior/isotope-source-search/Rb" in index["routes"]
    assert "GET /atom/behavior/isotope-candidate-admission/O" in index["routes"]
    assert "GET /scoring/readiness/Rb" in index["routes"]
    assert "GET /evidence/isotopes/C?mass_number=14" in index["routes"]
    assert "GET /evidence/isotopes/unresolved/Rb" in index["routes"]
    ...
    assert health["atom_behavior_profile_count"] == 101
    assert health["atom_behavior_gap_count"] == 82
    assert health["atom_behavior_gap_work_item_count"] == 82
    assert health["isotope_source_policy_count"] == 18
    assert health["isotope_source_search_receipt_count"] == 18
...
def test_local_api_exposes_atom_behavior_profiles():
    ...
    assert profiles["validation"]["profile_count"] == 101
    assert profiles["validation"]["radioisotope_profile_count"] == 5
