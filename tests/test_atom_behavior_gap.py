...
def test_atom_behavior_gap_receipts_make_missing_profile_coverage_explicit():
    ...
    scandium = get_atom_behavior_gap_receipt("Sc")
    radon = get_atom_behavior_gap_receipt("Rn")

    assert validation["validation_status"] == "atom_behavior_gap_receipts_validated"
    assert validation["receipt_count"] == 98
    assert validation["isotope_only_gap_count"] == 34
    assert validation["seed_and_matter_gap_count"] == 64
    assert validation["no_guess_policy_count"] == 98
    assert scandium.receipt_id == "MSPEE-ATOM-BEHAVIOR-GAP-Z021-Sc"
    assert scandium.target_unresolved_isotope_receipt_id == (
        "MSPEE-Z021-Sc-isotope_evidence-unresolved"
    )
    assert scandium.profile_blockers == ("isotope_evidence",)
    assert "stable_isotope_list" in scandium.missing_evidence
    assert scandium.no_guess_policy is True
...
def test_atom_behavior_gap_work_items_prioritize_buildable_seed_span_first():
    ...
    scandium = get_atom_behavior_gap_work_item("Sc")
    radon = get_atom_behavior_gap_work_item("Rn")

    assert validation["validation_status"] == "atom_behavior_gap_work_items_validated"
    assert validation["work_item_count"] == 98
    assert validation["isotope_evidence_required_count"] == 34
    ...
    assert items[0].symbol == "Sc"
    assert scandium.priority_rank == 0
    assert scandium.work_status == "isotope_evidence_required"
...
def test_local_api_exposes_atom_behavior_gap_routes():
    ...
    scandium_gap = handle_api_request("GET", "/atom/behavior/gaps/Sc")
    workplan = handle_api_request("GET", "/atom/behavior/workplan")
    radon_work = handle_api_request("GET", "/atom/behavior/workplan/Rn")

    assert gaps.status_code == 200
    assert gaps.payload["validation"]["receipt_count"] == 98
    assert scandium_gap.status_code == 200
    assert scandium_gap.payload["receipt"]["profile_blockers"] == ["isotope_evidence"]
    assert workplan.status_code == 200
    assert workplan.payload["validation"]["work_item_count"] == 98
...
def test_element_cli_prints_atom_behavior_gap_and_workplan(capsys):
    ...
    assert gap_output["receipts"][0]["symbol"] == "Sc"
