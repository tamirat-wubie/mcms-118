...
def test_element_readiness_scores_partition_ready_and_blocked_elements():
    ...
    scandium = get_element_readiness_score("Sc")
    ...
    assert validation["ready_count"] == 20
    assert validation["blocked_by_isotope_evidence_count"] == 34
    assert validation["blocked_by_seed_and_matter_count"] == 64
    assert validation["high_priority_gap_count"] == 34
    ...
    assert scandium.readiness_status == "atom_behavior_blocked_by_isotope_evidence"
    assert scandium.source_policy_id == "MSPEE-ISOTOPE-SOURCE-POLICY-Z021-Sc"
    assert scandium.gap_priority_score == 1.0
    assert scandium.constraint_tension_score == 0.9
...
def test_local_api_exposes_readiness_score_routes():
    ...
    assert scandium.payload["score"]["readiness_status"] == (
        "atom_behavior_blocked_by_isotope_evidence"
    )
    assert scandium.payload["score"]["source_policy_id"] == (
        "MSPEE-ISOTOPE-SOURCE-POLICY-Z021-Sc"
    )
...
def test_element_cli_prints_readiness_scores(capsys):
    ...
    assert output["scores"][0]["symbol"] == "Sc"
