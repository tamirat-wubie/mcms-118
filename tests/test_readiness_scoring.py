...
def test_element_readiness_scores_partition_ready_and_blocked_elements():
    ...
    gallium = get_element_readiness_score("Ga")
    ...
    assert validation["ready_count"] == 30
    assert validation["blocked_by_isotope_evidence_count"] == 24
    assert validation["blocked_by_seed_and_matter_count"] == 64
    assert validation["high_priority_gap_count"] == 24
    ...
    assert gallium.readiness_status == "atom_behavior_blocked_by_isotope_evidence"
    assert gallium.source_policy_id == "MSPEE-ISOTOPE-SOURCE-POLICY-Z031-Ga"
    assert gallium.gap_priority_score == 1.0
    assert gallium.constraint_tension_score == 0.9
...
def test_local_api_exposes_readiness_score_routes():
    ...
    assert gallium.payload["score"]["readiness_status"] == (
        "atom_behavior_blocked_by_isotope_evidence"
    )
    assert gallium.payload["score"]["source_policy_id"] == (
        "MSPEE-ISOTOPE-SOURCE-POLICY-Z031-Ga"
    )
...
def test_element_cli_prints_readiness_scores(capsys):
    ...
    assert output["scores"][0]["symbol"] == "Ga"
